  ...
  ws._write = function (incomingBuffer, enc, next) {
    var resumeNext = function() {
      ...
      if (receivedBuffersLength < partSizeThreshold) {
        ws.logger.info('_write: resume source stream');
        ws.sourceStream.resume();
        next();
      } else {
        uploadHandler(resumeNext);
      }
    };
    ...
    function write() {
      ...
      if (receivedBuffersLength < partSizeThreshold) {
        ...
        ws.logger.info('_write: pause source stream');
        ws.sourceStream.pause();
      }
